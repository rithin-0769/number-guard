import { Download, FileJson, Lock, Plus, ShieldCheck, Trash2, Upload } from "lucide-react";
import { useRef } from "react";
import { cn } from "../utils/cn";
import { fmtPhone, scoreLabel } from "../lib/storage";
import type { Vault } from "../types";
import type { VaultMeta } from "./VaultGate";
...
export function Sidebar({
  ...
  const report = vault.services.filter((s) => s.status === "done").length;
  ...
  return (
    <div className="flex h-full flex-col gap-5 overflow-y-auto p-4">
      ...
      <div className="flex items-center gap-3 px-1 pt-1">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-mint-500 text-ink-950 shadow-[0_0_24px_rgba(43,207,140,0.4)]">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 truncate font-mono text-[9.5px] uppercase tracking-[0.2em] text-faint">
            SIM recycle risk console
          </p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-line bg-ink-850 p-3.5">
        <div className="flex items-center justify-between">
          ...
          <span className={cn("h-1.5 w-1.5 rounded-full", exposure === 0 ? "bg-mint-400" : daysLeft <= 14 ? "bg-flare-500 pulse-dot" : "bg-mint-500/50")} />
        </div>
        <p className="mt-2 font-mono text-lg font-bold tracking-wider text-paper">{fmtPhone(vault.phone)}</p>
        <div className="mt-3">
          <div className="mb-1.5 flex items-center justify-between text-[11px]">
            <span className="text-mist">
              {report}/{total} cleared
            </span>
            <span className={cn("font-mono font-semibold", exposure === null ? "text-faint" : `text-${sl.tone === "none" ? "mist" : sl.tone}-300`)}>
              {exposure === null ? "—" : `${exposure}%`} {sl.text.toLowerCase()}
            </span>
          </div>
          <Progress value={total === 0 ? 0 : report / total} tone="mint" />
        </div>
        {vault.newNumber && (
